<template>
  <div class="page20-container">
    <div class="page20-frame40">
      <div class="page20-group39">
        <div class="page20-container1">
          <div class="page20-container2">
            <div class="page20-container3">
              <div class="page20-container4">
                <div class="page20-rectangle4">
                  <div class="page20-group78">
                    <img
                      src="https://aheioqhobo.cloudimg.io/v7/_playground-bucket-v2.teleporthq.io_/58334886-fac5-4923-ae0b-527c66bbc2dd/e76b260b-1b54-4016-bb07-6b33ef58a2db?org_if_sml=1124275"
                      alt="IMAGE213553"
                      class="page20-image21"
                    />
                    <img
                      src="https://aheioqhobo.cloudimg.io/v7/_playground-bucket-v2.teleporthq.io_/58334886-fac5-4923-ae0b-527c66bbc2dd/0f244bf9-4985-4d5f-8287-184f47044347?org_if_sml=1101133"
                      alt="IMAGE313553"
                      class="page20-image31"
                    />
                    <img
                      src="https://aheioqhobo.cloudimg.io/v7/_playground-bucket-v2.teleporthq.io_/58334886-fac5-4923-ae0b-527c66bbc2dd/ee5ff1ab-e5ca-493c-8343-8aaa4817d223?org_if_sml=168931"
                      alt="IMAGE413554"
                      class="page20-image41"
                    />
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="page20-group38">
          <div class="page20-frame4">
            <router-link to="/1211212111" class="page20-navlink">
              <div class="page20-group1"><span class="page20-text">3</span></div>
            </router-link>
            <div class="page20-group2">
              <img
                src="/playground_assets/rectangle43135-t35h-1100w.png"
                alt="Rectangle73137"
                class="page20-rectangle7"
              />
              <span class="page20-text1">10</span>
            </div>
            <div class="page20-component1">
              <img
                src="/playground_assets/rectangle43135-t35h-1100w.png"
                alt="Rectangle83137"
                class="page20-rectangle8"
              />
              <span class="page20-text2">6</span>
            </div>
            <div class="page20-group3">
              <img
                src="/playground_assets/rectangle41536-62and-200h.png"
                class="page20-rectangle9"
              />
              <span class="page20-text3">2</span>
            </div>
          </div>
        </div>
      </div>
      <router-link to="/1" class="page20-navlink1">
        <img
          src="https://aheioqhobo.cloudimg.io/v7/_playground-bucket-v2.teleporthq.io_/58334886-fac5-4923-ae0b-527c66bbc2dd/9163f27a-5c34-4efb-919b-2d66ffefbbe0?org_if_sml=128923"
          alt="home3136"
          class="page20-home"
        />
      </router-link>
    </div>
  </div>
</template>

<script>
export default {
  name: 'Page20',
  metaInfo: {
    title: 'Page20 - exported project',
    meta: [
      {
        property: 'og:title',
        content: 'Page20 - exported project',
      },
    ],
  },
}
</script>

<style scoped>
.page20-container {
  width: 100%;
  display: flex;
  overflow: auto;
  min-height: 100vh;
  align-items: center;
  flex-direction: column;
  background-size: cover;
  justify-content: center;
  background-image: url("/playground_assets/09469961-2df6-4da0-9c7f-759b2530270e-q1as-1500w.undefined");
}
.page20-frame40 {
  gap: 60px;
  width: 1230px;
  height: auto;
  display: flex;
  align-self: center;
  align-items: flex-start;
  justify-content: flex-end;
}
.page20-group39 {
  width: 1057px;
  height: 896px;
  display: flex;
  position: relative;
  align-items: center;
  flex-shrink: 1;
  justify-content: center;
}
.page20-container1 {
  flex: 0 0 auto;
  width: 1057px;
  height: auto;
  display: flex;
  align-items: center;
  flex-direction: column;
}
.page20-container2 {
  flex: 0 0 auto;
  width: 1057px;
  height: auto;
  display: flex;
  align-items: center;
  flex-direction: column;
}
.page20-container3 {
  flex: 0 0 auto;
  width: 1057px;
  height: auto;
  display: flex;
  align-items: center;
  flex-direction: column;
}
.page20-container4 {
  flex: 0 0 auto;
  width: 1057px;
  height: auto;
  display: flex;
  align-items: center;
  flex-direction: column;
}
.page20-rectangle4 {
  right: -2px;
  width: 1057px;
  bottom: -4px;
  height: 896px;
  display: flex;
  position: absolute;
  align-items: center;
  border-color: var(--dl-color-primary-text);
  border-width: 5px;
  border-radius: 15px;
  background-size: cover;
  justify-content: center;
  background-image: url("/playground_assets/rectangle43135-t35h-1100w.png");
  background-position: center;
}
.page20-group78 {
  width: 977px;
  height: 618px;
  display: flex;
  position: relative;
  align-self: flex-start;
  margin-top: 20px;
  align-items: flex-start;
  flex-shrink: 1;
}
.page20-image21 {
  top: 0px;
  left: 0px;
  width: 323px;
  height: 528px;
  position: absolute;
}
.page20-image31 {
  top: 20px;
  left: 688px;
  width: 289px;
  height: 421px;
  position: absolute;
}
.page20-image41 {
  top: 264px;
  left: 323px;
  width: 373px;
  height: 354px;
  position: absolute;
}
.page20-group38 {
  top: 640px;
  left: 47px;
  width: 964px;
  height: 220px;
  display: flex;
  position: absolute;
  align-items: flex-start;
  flex-shrink: 1;
}
.page20-frame4 {
  gap: 60px;
  top: 0px;
  left: 0px;
  width: 964px;
  height: 220px;
  display: flex;
  position: absolute;
  align-items: flex-start;
}
.page20-navlink {
  display: contents;
}
.page20-group1 {
  top: 0px;
  left: 0px;
  width: 196px;
  height: 196px;
  display: flex;
  position: absolute;
  align-items: flex-start;
  flex-shrink: 1;
  border-color: var(--dl-color-primary-text);
  border-width: 5px;
  border-radius: var(--dl-radius-radius-radius15);
  background-size: cover;
  text-decoration: none;
  background-image: url("/playground_assets/rectangle41536-wyuo-200h.png");
}
.page20-text {
  top: -19px;
  left: 43px;
  color: rgb(16, 55, 154);
  width: 104px;
  height: auto;
  position: absolute;
  font-size: 200px;
  font-style: Bold;
  text-align: left;
  font-family: Frozen1;
  font-weight: 700;
  line-height: normal;
  font-stretch: normal;
  text-decoration: none;
}
.page20-group2 {
  top: 0px;
  left: 256px;
  width: 196px;
  height: 220px;
  display: flex;
  position: absolute;
  align-items: flex-start;
  flex-shrink: 1;
}
.page20-rectangle7 {
  top: 0px;
  left: 0px;
  width: 196px;
  height: 196px;
  position: absolute;
  border-color: rgba(16, 55, 154, 1);
  border-style: solid;
  border-width: 5px;
  border-radius: 15px;
}
.page20-text1 {
  top: -16px;
  left: -5px;
  color: rgb(16, 55, 154);
  height: auto;
  position: absolute;
  font-size: 200px;
  font-style: Bold;
  text-align: left;
  font-family: Frozen1;
  font-weight: 700;
  line-height: normal;
  font-stretch: normal;
  text-decoration: none;
}
.page20-component1 {
  top: 0px;
  left: 512px;
  width: 196px;
  height: 220px;
  display: flex;
  position: absolute;
  align-items: flex-start;
  flex-shrink: 0;
}
.page20-rectangle8 {
  top: 0px;
  left: 0px;
  width: 196px;
  height: 196px;
  position: absolute;
  border-color: rgba(16, 55, 154, 1);
  border-style: solid;
  border-width: 5px;
  border-radius: 15px;
}
.page20-text2 {
  top: -14px;
  left: 45px;
  color: rgb(16, 55, 154);
  height: auto;
  position: absolute;
  font-size: 200px;
  font-style: Bold;
  text-align: left;
  font-family: Frozen1;
  font-weight: 700;
  line-height: normal;
  font-stretch: normal;
  text-decoration: none;
}
.page20-group3 {
  top: 0px;
  left: 768px;
  width: 196px;
  height: 220px;
  display: flex;
  position: absolute;
  align-items: flex-start;
  flex-shrink: 1;
  text-decoration: none;
}
.page20-rectangle9 {
  top: 0px;
  left: 0px;
  width: 196px;
  height: 196px;
  position: absolute;
  border-color: rgba(16, 55, 154, 1);
  border-style: solid;
  border-width: 5px;
  border-radius: 15px;
}
.page20-text3 {
  top: -16px;
  left: 54px;
  color: rgb(16, 55, 154);
  height: auto;
  position: absolute;
  font-size: 200px;
  font-style: Bold;
  text-align: left;
  font-family: Frozen1;
  font-weight: 700;
  line-height: normal;
  font-stretch: normal;
  text-decoration: none;
}
.page20-navlink1 {
  display: contents;
}
.page20-home {
  width: 113px;
  height: 116px;
  border-color: rgba(16, 55, 154, 1);
  border-style: solid;
  border-width: 5px;
  border-radius: 15px;
  text-decoration: none;
}
</style>
